	...
	for k, v in pairs(RPExtraTeams) do
		if !v.type || v.type == SELECTED_TYPE then
			...
			elseif v.requireUnlock && !table.HasValue(Job.Unlocks, RPExtraTeams[v.requireUnlock].command) then
					desc = "You need to unlock "..RPExtraTeams[v.requireUnlock].name.." first."
					panel:OnClick(function() LocalPlayer():ConCommand("Job.Unlock "..v.command) end)
			else
				desc = "Unclock for "..v.unlockCost.."$."
				panel:OnClick(function()
					local list = vgui.Create("MList")
					list:AddItem("Are you sure?")
					list:AddItem("")
					list:AddItem("Yes, I am!", function()
						LocalPlayer():ConCommand("Job.Unlock "..v.command)
					end)
					list:AddItem("No.", function() end)
				end)
			end
...
concommand.Add("JonbNPCSpawnPos", function(ply, cmd, args)
	...
	local outpus = [[
			[n] = {
				...
				angle = Angle(0,]]..ang.y..[[,0),
			}
]]

	LocalPlayer():PrintMessage(HUD_PRINTCONSOLE, outpus)
	LocalPlayer():PrintMessage(HUD_PRINTCONSOLE, "Copied to clipboard")

	SetClipboardText(outpus)
end)
